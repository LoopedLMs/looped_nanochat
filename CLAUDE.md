...
Paper summaries and experiment logs are synced to a Notion database. The project page ID is in `./notion.txt`. If this file does not exist, use `notion-search` to find the "Looped Nanochat" page whose ancestor path includes "Loop Transformers → Experimentation".

When the user references a paper or asks about related work, check Notion first using `notion-search` before re-reading local files or re-downloading.
