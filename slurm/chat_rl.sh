...
# --- Model config ---
INIT_SOURCE=sft_latent              # checkpoint source: base, sft, sft_latent, rl
MODEL_TAG=r4_1.35e19_s20_warmup    # model tag to load from
OUTPUT_TAG=r4_1.35e19_s20_warmup_rl # output tag (defaults to MODEL_TAG if empty)
...
torchrun --standalone --nproc_per_node=$NPROC_PER_NODE -m scripts.chat_rl \
    -- --init-source $INIT_SOURCE --model-tag $MODEL_TAG --output-tag $OUTPUT_TAG \
    --run $RUN \
